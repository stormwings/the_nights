...
const App = () => {
  ...
  const [landContract, setLandContract] = React.useState(null);
  const [cost, setCost] = React.useState(null);
  const [buildings, setBuildings] = React.useState(null);
  const [landId, setLandId] = React.useState(null);
  const [landName, setLandName] = React.useState(null);
  const [landOwner, setLandOwner] = React.useState(null);
  ...
  const loadBlockchainData = async () => {
    if (typeof window.ethereum !== "undefined") {
      ...
      const { address } = Land.networks[networkId];

      const land = new web3.eth.Contract(Land.abi, address);
      setLandContract(land);

      const cost = await land.methods.initialCost().call();
